...
def newLogger(cfg):
    ...
    while True:
        thisdir = os.path.join(cfg.get(
            'logdir', 'log'), datetime.datetime.now().strftime("%Y%m%d%H%M%S_%f_") + client_ip)
...
class ThreadedLogRequestHandler(SocketServer.BaseRequestHandler):
    ...
    def setup(self):
        ...
        ThreadedLogRequestHandler.stdlog.debug(
            '%s:%d accept' % self.client_address)
    ...
    def handle(self):
        while True:
            d = self.request.recv(
                ThreadedLogRequestHandler.cfg.get('bufsize', 256 * 1024))
            if not d:
                break
            self.data = self.data + d
            i = self.data.find(
                ThreadedLogRequestHandler.cfg.get('separator', '\n'))
            while i != -1:
                self.log.debug(self.data[:i])
                self.data = self.data[i + len(
                    ThreadedLogRequestHandler.cfg.get('separator', '\n')):]
                i = self.data.find(
                    ThreadedLogRequestHandler.cfg.get('separator', '\n'))
    ...
    def finish(self):
        ThreadedLogRequestHandler.stdlog.debug(
            '%s:%d closed' % self.client_address)
        self.log.debug(self.data)
        self.data = ''
...
def main():
    ...
    stdlog.debug(cfg.get('ip', '0.0.0.0') + ':' + str(
        cfg.get('port', 6211)) + '  serving...')
